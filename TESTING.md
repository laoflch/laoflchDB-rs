...
- **测试场景**:
  ...
  3. 上传 JPEG 图片
  4. 上传图片自动生成 key（key 为空时，验证生成 Snowflake ID）
  5. Snowflake ID 唯一性与单调递增（连续上传 5 张，验证 ID 唯一且单调递增）
  6. Snowflake key 完整生命周期（用自动生成的 key 获取原图/元数据/三种缩略图）
  7. Snowflake key 列出与删除（验证出现在列表中、删除级联、删除后不可访问）
  8. 上传图片带自定义元数据
  9. 覆盖上传同名图片（验证 ETag 更新）
  10. 上传大图片 1000x1000
  11. 上传长方形图片（验证 cover/contain 缩放模式）
  12. 上传无效图片数据（应返回 INVALID_ARGUMENT）
  13. 使用默认 bucket 上传
  14. 获取原图
  15. 获取不存在图片（应失败）
  16. 获取 thumbnail 缩略图（验证 128x128）
  17. 获取 small 缩略图（验证不超过 256x256）
  18. 获取 medium 缩略图（验证不超过 512x512）
  19. 获取无效 size 缩略图（应返回 INVALID_ARGUMENT）
  20. 验证大图片缩略图尺寸（1000x1000 → medium 512x512）
  21. 验证 thumbnail cover 模式（非正方形图片裁剪为 128x128）
  22. 获取图片元数据
  23. 获取不存在图片元数据（应返回 NOT_FOUND）
  24. 列出图片
  25. 带前缀列出图片
  26. 删除图片（验证级联删除原图+缩略图+元数据）
  27. 验证删除后不可访问
  28. 删除不存在图片（幂等）
...
- **测试场景**:
  ...
  3. 上传 PNG 图片
  4. 上传 JPEG 图片
  5. 上传图片自动生成 key（验证生成 Snowflake ID 纯数字）
  6. Snowflake ID 唯一性与单调递增（连续上传 5 张）
  7. Snowflake key 完整生命周期（用自动生成的 key 获取原图/元数据/三种缩略图）
  8. Snowflake key 列出与删除（验证列表中出现、删除级联、删除后 404）
  9. 上传带元数据图片
  10. 上传大图片 1000x1000
  11. 上传长方形图片（验证缩略图尺寸）
  12. 上传无效图片数据（应返回 400）
  ...
  17. 获取 small 缩略图
  18. 获取 medium 缩略图
  19. 获取无效 size 缩略图（应返回 400）
  20. 获取图片元数据
  21. 获取不存在图片元数据（应返回 404）
  22. 列出图片
  23. 带前缀列出图片
  24. 删除图片
  25. 验证删除后不可访问
  26. 删除不存在图片（幂等）
  27. 验证长方形图片 thumbnail cover 模式和 small contain 模式
